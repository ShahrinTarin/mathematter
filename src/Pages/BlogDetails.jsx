...
const BlogDetails = () => {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] pb-12 flex flex-col items-center justify-center bg-white dark:bg-gray-900 transition-colors duration-500">
      ...
      {/* Comments Section */}
      <section className="lg:w-1/2 w-full bg-base-200 py-12 rounded-lg shadow-2xl px-7 mt-10 dark:bg-gray-800 dark:text-gray-100">
        <h2 className="text-3xl font-semibold mb-6 text-gray-700 dark:text-gray-200">Comments</h2>

        {blog.email === user?.email ? (
          <p className="text-red-500 text-sm mb-6">You can't comment on your own blog</p>
        ) : (
          <form onSubmit={handleComments} className="mb-8 space-y-4">
            <div className="flex items-center space-x-4 mb-2">
              ...
              <textarea
                ...
                className="textarea textarea-accent w-full resize-none bg-white dark:bg-gray-700 dark:text-gray-100 border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-[#1b9c85]"
                rows={4}
              ></textarea>
            </div>
            <div className="text-right">
              <button
                type="submit"
                className="inline-block px-6 py-2 rounded-md bg-gradient-to-br from-[#1b9c85] to-gray-600 text-white font-semibold shadow-md hover:from-[#159972] hover:to-gray-700 transition"
              >
                Comment
              </button>
            </div>
        ...
        <p className="text-xs text-[#1b9c85] mb-4">{comments.length} people commented on this blog</p>

        <div className="space-y-4 max-h-[400px] overflow-auto">
          {comments.map((singleComment) => (
            <article
              key={singleComment._id}
              className="flex items-start gap-4 p-4 rounded-md bg-white dark:bg-gray-700 shadow-sm"
            >
              ...
              <div>
                <h3 className="font-semibold text-gray-900 dark:text-gray-100">{singleComment.userName}</h3>
                <p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{singleComment.comment}</p>
              </div>
            </article>
          ))}
        </div>
